...
def train(train_dataset, val_dataset, epochs, hyperparams, tuning=False):
    ...
    for epoch in range(num_epochs):
        ...
        if not tuning and epoch % 1000 == 0:
            print(
                f"Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_losses[-1]}, "
    ...
    if not tuning:
        print(
            f"Training complete. Best model saved to {best_model_path} with validation loss {best_val_loss}."
        )
...
if __name__ == "__main__":
    ...
    train(train_dataset, val_dataset, epochs=2000, hyperparams=hyperparameters)
